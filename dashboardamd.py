...
# Local calls (amd-smi) are cheap — keep them fast.
# Network calls can be staggered to reduce overhead.
REFRESH_GPU = 1         # amd-smi query (local, ~10ms)
...
def get_main_model_decode(valid_metrics, running_models, gpus):
    ...
    # Check if llama-server is actively computing (via amd-smi or nvidia-smi)
    is_decoding = False
    gpu_util = 0
    try:
        result = subprocess.run(
